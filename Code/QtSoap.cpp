...
bool QtSoapMessage::setContent(const QByteArray &buffer)
{
    ...
    {
        QString sep("------=");
        qDebug() << "Multi-Part Http Response " << buffer.size();
        // lets read the first part
        int part1 = buffer.indexOf(sep, 0);
        int epart1 = buffer.indexOf('\n',sep.size()+part1);
        int startXML = buffer.indexOf("<env", epart1);
        int part2 = buffer.indexOf(sep,sep.size()+part1);
        QByteArray buf(&buffer.data()[startXML], part2-startXML);
        bool res = setContent(buf);
        // Soap Message Readed
        // Binary set
        int cid = buffer.indexOf("Content-Id:", part2);
        int lfeed = buffer.indexOf("\r\n", cid);
        int lfeed2 = buffer.indexOf("\r\n", lfeed+2);
        binary = QByteArray(&buffer.data()[lfeed2+2], buffer.size()-lfeed2+2);
        return res;
    }
    else
    {
...
const QByteArray& QtSoapMessage::getBinary() const
{
    return binary;
}
